	...
	bool showUI, std::uint16_t interactingBlock, ChestBlock *chestBlock)
{
	...
	if (glui::drawButton(renderer2d, guideButton, Colors_White, "FIELD GUIDE",
	...
	panel.w *= 0.72f;
	renderer2d.render9Patch(panel, 24, {0.25f, 0.19f, 0.11f, 0.96f}, {}, 0.f,
		buttonTexture, GL2D_DefaultTextureCoords, {0.2f, 0.8f, 0.8f, 0.2f});
	...
	auto drawText = [&](float x, float y, const std::string &text, float size, glm::vec4 color = Colors_White)
	{
		renderer2d.renderText({x, y}, text.c_str(), font, color, size, 3, 0, false);
	};
	...
	if (!showGuideCrafts)
	{
		...
		if (glui::drawButton(renderer2d, pageButton, Colors_White, "CRAFTING GUIDE",
			font, buttonTexture, platform::getRelMousePosition(),
			platform::isLMouseHeld(), platform::isLMouseReleased()))
		{
	...
	else
	{
		...
		if (glui::drawButton(renderer2d, pageButton, Colors_White, "BACK TO OBJECTIVES",
			font, buttonTexture, platform::getRelMousePosition(),
			platform::isLMouseHeld(), platform::isLMouseReleased()))
		{
			showGuideCrafts = false;
